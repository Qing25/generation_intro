...
class CopyLogitsProcessor(LogitsProcessor):
    """ CopyLogitsProcesss 只用来获取encoder_input_ids """
    def __init__(self, encoder_input_ids: torch.LongTensor, copy_fn=None):
...
class ExclusiveLogitsProcessor(LogitsProcessor):
    def __init__(self, encoder_input_ids):
        self.encoder_input_ids = encoder_input_ids
        self.cypher_keyword_ids = []
...
class CopyMechanismMixIn(GenerationMixin):
    """
        使用时需要在模型中实现 apply_copy_mechanism()

        在generate() 函数中传入 `enable_copy=True` 和 `output_attentions=True`
    """
    @torch.no_grad()
    def generate(
        self,
        ...
        **model_kwargs,
        ...
        ```"""
        enable_copy = model_kwargs.get("enable_copy", None)
        if enable_copy:
            model_kwargs.pop('enable_copy')
        ...
        if is_greedy_gen_mode:
            ...
            ##################################################
            #     Copy mechanism by enable_copy=True
            ##################################################
            if enable_copy:
                
                logits_processor.append(
                    CopyLogitsProcessor(encoder_input_ids=inputs_tensor)
                )

                return self.greedy_search_with_copy(
                    input_ids,
                    logits_processor=logits_processor,
    ...
    def greedy_search_with_copy(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            ##################################################
            # a. 取出存放的输入序列 
            encoder_input_ids = logits_processor[-1].encoder_input_ids
            next_token_logits = self.apply_copy_mechanism(
                next_token_logits, 
                outputs.cross_attentions,
                outputs.decoder_hidden_states,
                encoder_input_ids
            ).squeeze(1)                 # (bsz, vocab_size)
